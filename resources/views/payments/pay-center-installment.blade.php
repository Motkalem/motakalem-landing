...
<head>
    <meta charset="utf-8">
    <title>متكلم - دفع القسط  - مركز متكلم</title>
    <base href="/">
...
<script
    src="{{env('RYD_HYPERPAY_URL')}}/paymentWidgets.js?checkoutId={{$checkoutId??data_get($_GET,'checkoutId')}}"
    integrity="{{$integrity}}"
    ...
    <div style="height: 650px;padding-top:50px;  margin-bottom:0px; direction: rtl; ">
        ...
        @if(data_get($_GET,'brand'))
            @if(in_array(data_get($_GET,'brand'), ['visa', 'master','mada',  'applepay'] ))
                ...
                <form action="{{'/pay-center-installment/result/'.request()->instId.'/'.strtoupper( data_get($_GET,'brand')).'/'}}"
            ...
            @else
                <form action="{{'/pay-center-installment/result/'.request()->instId.'/'.strtoupper( data_get($_GET,'brand')).'/'}}" class="paymentWidgets"
