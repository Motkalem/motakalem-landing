...
<head>
    ...
    <meta http-equiv="Content-Security-Policy"
          content="
                       ...
                       frame-src 'self'  https://oppwa.com {{env('SNB_HYPERPAY_WIDGET_URL')}} https://geoissuer.cardinalcommerce.com/ https://eu-prod.ppipe.net/ https://authentication.cardinalcommerce.com/ https://admin.motkalem.sa https://staging-admin.motkalem.sa;
...
<script
    src="{{env('SNB_HYPERPAY_URL')}}/paymentWidgets.js?checkoutId={{$checkoutId??data_get($_GET,'checkoutId')}}"
    integrity="{{$integrity}}"
...
<div class="routing">
    ...
    <div style="height: 650px;padding-top:50px;  margin-bottom:0px; direction: rtl; ">
        <h1 class="text-center" style="text-align: center">ستقوم بدفع مبلغ {{ $installment?->installment_amount }}
        ...
        @if(data_get($_GET,'brand'))
            @if(in_array(data_get($_GET,'brand'), ['visa', 'master','mada',  'applepay'] ))
                ...
                <form action="{{'/pay-installment/result/'.request()->instId.'/'.strtoupper( data_get($_GET,'brand')).'/'}}"
            ...
            @else
                <form action="{{'/pay-installment/result/'.request()->instId.'/'.strtoupper( data_get($_GET,'brand')).'/'}}" class="paymentWidgets"
